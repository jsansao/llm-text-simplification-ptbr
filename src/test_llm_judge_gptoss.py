...
CACHE_DIR = Path(__file__).resolve().parent.parent / "cache2_gptoss_test"
...
def main():
    print("=" * 60)
    print(f"TESTE: GPT-OSS-120B como juiz")
    ...
    out_path = RESULTS_DIR / "llm_judge_results_gptoss_test.json"
    ...
    if gpt_path.exists():
        ...
        for model in MODEL_ORDER:
            ...
            for d in DIMENSIONS:
                ...
                diff = ds_avg - gpt_avg
                line += f"    {d}: OSS={ds_avg:.2f}  GPT={gpt_avg:.2f}  diff={diff:+.2f}\n"
            print(line)
