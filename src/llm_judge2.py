...
CACHE_DIR = Path(__file__).resolve().parent.parent / "cache2"
...
# Thread-local storage for per-thread LLMClient instances
...
def process_task(task: tuple) -> dict | None:
    ...
    if ratings is None:
        print(f"  [run {run}] Failed to parse for {model_display}: {response[:100]}", flush=True)
        return None
...
def main():
    ...
    print(f"Judge: {JUDGE_MODEL} (temperature={JUDGE_TEMPERATURE}, {N_RUNS} runs, {MAX_WORKERS} workers)")
    ...
    if remaining:
        ...
        def task_wrapper(t):
            ...
            with done_lock:
                ...
                if done_count % 50 == 0 or done_count == len(remaining):
                    print(f"  {done_count}/{len(remaining)}", flush=True)
                if result is None:
                    fail_count += 1
                else:
        ...
        print(f"\nDone. Success: {len(all_results)}, Failed: {fail_count}")
        print(f"Reading all results from cache...")

    # Read all results from cache for consistency
    all_results = []
    for t in tasks:
    ...
    out_path = RESULTS_DIR / "llm_judge_results_gpt4omini.json"
