...
\author{
  ...
  \instituto{Departamento de Tecnologia em Engenharia Civil, Computação,
             Automação, Telemática e Humanidades (DTECH) \\
             Universidade Federal de São João del-Rei (UFSJ) \\
             Ouro Branco, MG, Brasil}
  \email{joao@ufsj.edu.br}
  \and
  Michel Carlo Rodrigues Leles
  \href{https://orcid.org/0000-0001-7399-7444}{0000-0001-7399-7444}
  \instituto{Departamento de Tecnologia em Engenharia Civil, Computação,
             Automação, Telemática e Humanidades (DTECH) \\
             Universidade Federal de São João del-Rei (UFSJ) \\
             Ouro Branco, MG, Brasil}
  ...
  \instituto{Programa de Pós-Graduação em Ciência da Computação (PPGCC) \\
             Universidade Federal de São João del-Rei (UFSJ) \\
...
legibilidade (Flesch $\Delta\mathrm{F} = +13,2$, Fog $\Delta\mathrm{Fog} = -4,0$).
 Em similaridade lexical, o ajuste em domínio do PTT5-small
(60M) supera o prompting de modelos 100$\times$ maiores
(ROUGE-1 até 0,74, SARI até 55,5), mas com
 comportamento conservador (R1$_{OP} \approx 0{,}93$,
$\Delta \mathrm{F} \leq +4{,}2$) que limita os ganhos de
legibilidade e sugere que, em tarefas de natureza lexical, a
escala do modelo não substitui o ajuste em domínio. A estratégia
chain-of-thought reduz a similaridade sem benefícios de
legibilidade nos quatro modelos testados, indicando que a
tarefa não se beneficia de raciocínio explícito. Uma avaliação
complementar LLM-as-judge com dois juízes (Gemini 3.1 Flash Lite
e GPT-4o mini) confirma a fluência e adequação das
simplificações, mas revela baixa concordância entre juízes
($\alpha = 0{,}181$ em média) e sensibilidade ao modelo juiz
escolhido.
\end{resumo}
...
readability indices adapted for Portuguese. Results show that
Claude Sonnet 4.6 (few-shot, SARI 42.6) and Qwen 2.5 7B (few-shot,
SARI 41.7) achieve the best similarity scores among the prompting
configurations, both reaching ROUGE-1 = 0.66, while Llama 4 Scout obtains the
largest readability gains (Flesch $\Delta\mathrm{F} = +13.2$, Fog $\Delta\mathrm{Fog} = -4.0$).
In similarity, domain-adjusted PTT5-small (60M) outperforms prompting
of models 100$\times$ larger (ROUGE-1 up to 0.74, SARI up to 55.5), but
with extreme conservatism (R1$_{OP} \approx 0.93$, $\Delta \mathrm{F} \leq +4.2$)
that caps readability gains and suggests that, for lexical tasks, model
scale does not substitute domain adjustment. The chain-of-thought
strategy reduces similarity without readability benefits in the four
tested models, indicating that the task does not benefit from explicit
reasoning. A complementary LLM-as-judge evaluation with two judge models
...
pessoas com baixa escolaridade, pessoas com afasia, pessoas com
déficit de atenção e aprendizes de português como segunda língua
\citep{aluisio2008}.
...
As contribuições deste trabalho são: (i) comparação controlada de
cinco LLMs (7B--17B) em três estratégias de prompting e de quatro
modelos ajustados em domínio (60M--0,5B), no mesmo dataset e nas
mesmas 200 amostras, permitindo contrastar diretamente os
paradigmas sob idênticas condições de avaliação; (ii) evidência
de que o ajuste em domínio do PTT5-small (60M) atinge similaridade
lexical superior à do prompting de modelos grandes
(ROUGE-1 até 0,74, SARI até 55,5), mas com
 comportamento conservador extremo
(R1$_{OP} \approx 0{,}93$, $\Delta \mathrm{F} \leq +4{,}2$),
sugerindo que, em tarefas de natureza lexical, a escala do modelo
não substitui o ajuste em domínio; (iii) demonstração de que a
estratégia chain-of-thought reduz a similaridade sem ganho de
legibilidade nos quatro modelos testados, indicando que a tarefa
de simplificação não se beneficia de raciocínio explícito;
(iv) demonstração de que a avaliação LLM-as-judge apresenta baixa
concordância entre juízes ($\alpha = 0{,}181$ em média), com
sensibilidade ao modelo juiz escolhido, o que questiona sua
confiabilidade como métrica única; e (v) disponibilização de
código, amostra de avaliação e adaptação dos índices de
legibilidade Flesch e Gunning-Fog ao português com contagem
silábica baseada em grupos vocálicos\footnote{%
...
Utilizamos o dataset PorSimplesSent versão pss2\footnote{%
  Arquivo: \texttt{pss2\_align\_length\_ori\_nat.tsv}.}, contendo
2.370 pares de frases original--simplificada. As frases originais têm média de 21,8
...
\citep{cohen1988}. O valor de Cohen (1988) é citado como
aproximação: ananálises formais de poder para bootstrap
pareado com correção de múltiplas comparações estão
...
identificado com esse problema.\footnote{Uma análise de
sensibilidade (recalculo de R-1 e SARI excluindo a amostra
\#16) confirma que o par não altera qualitativamente as
conclusões: Claude S. 4.6 (few-shot) passa de R-1 $= 0{,}661$
para $0{,}665$ ($\Delta = +0{,}003$) e SARI de $42{,}6$ para
$42{,}7$ ($\Delta = +0{,}04$); Qwen 2.5 7B (few-shot) passa
de R-1 $= 0{,}664$ para $0{,}667$ ($\Delta = +0{,}002$) e SARI
permanece em $41{,}7$.}
...
testados, tornando o custo extra de 200 chamadas de API
desnecessário para a análise. Consequentemente, a generalização
do achado sobre CoT limita-se a esses quatro modelos.
...
e zero amostras de treino, enquanto os ajustados em domínio têm 60M--0,5B
parâmetros e 1.953 amostras de treino. Modelos maiores ajustados em domínio
          ...
          {\tt Cópia} mostra o piso de similaridade (copiar o original
           como predição). A linha {\tt Ref.\ como pred.} mostra o
            SARI da referência humana contra o original (original como
             ...
             do texto fonte. Valores arredondados a 2 casas decimais;
             precisão completa na Tabela~\ref{tab:significancia}.}
...
\toprule
Modelo & Estrat. & Flesch & $\Delta \mathrm{F}$ & Fog & $\Delta \mathrm{Fog}$\\
\midrule
Original    & --- &  7,6 & ---   & 21,6 & --- \\
...
texto fonte. Vale notar que o SARI não possui, em si, um
teto formal: a métrica cresce com a distância controlada entre
a saída e a fonte. Assim, o valor 24,6 indica a magnitude da
divergência lexical entre a referência humana e o original, e
não um limite superior absoluto para a tarefa.
...
agressivas. A diferença de R1$_{OP}$ entre Qwen 2.5 7B
few-shot e Llama 4 Scout few-shot foi avaliada
por teste bootstrap pareado (10.000 iterações) e correção de
Holm-Bonferroni: diferença de $+0{,}123$ (IC95\%
$[0{,}098; 0{,}146]$, $p < 0{,}001$), confirmando que as
simplificações do Qwen 2.5 7B são significativamente mais
conservadoras que as do Llama 4 Scout.
...
humano com Flesch $\Delta \mathrm{F}$ de +12,9. O GPT-4o mini e o GPT-4o (full)
zero-shot também superam o humano ($\Delta \mathrm{F} = +11,1$ e +9,3,
respectivamente). O Qwen 2.5 7B few-shot, ao contrário, produz ganhos
mais modestos ($\Delta \mathrm{F} = +6,2$), sugerindo que suas simplificações
...
ajustados em domínio, comparando-os com o melhor prompting de cada modelo e com
a referência humana.
...
\toprule
Modelo & R-1 & R-L & BS & SARI & R1$_{OP}$ & BS$_{OP}$ & $\Delta \mathrm{F}$ & $\Delta \mathrm{Fog}$ \\
\midrule
...
é modesto ($\Delta \mathrm{F} = +2,6$), e a similaridade
...
mas maior ganho de legibilidade ($\Delta \mathrm{F} = +4,2$ vs.\ +2,6),
...
os três modelos são equivalentes nessas métricas. Em $\Delta \mathrm{F}$,
...
e $\Delta \mathrm{F} = +9,6$. As simplificações são menos similares
lexicalmente, mas os ganhos de legibilidade são substanciais, superando
o PTT5 em $\Delta \mathrm{F}$ (+9,6 vs.\ +2,6 a +4,2). Em uma inspeção
manual das 200 amostras, identificamos duas saídas em que o modelo
reproduz a instrução em vez de executá-la (metatexto, 1\%):
a amostra \#16 produz ``Simplifique o texto a seguir: [\ldots]''
e a amostra \#141 produz apenas ``Aqui está o texto
simplificado:''. Além disso, doze saídas (6\%) são cópias
literais do original. Esses casos ilustram uma fragilidade
conhecida do fine-tuning leve em modelos pequenos, mas não
alteram qualitativamente as métricas agregadas reportadas.

As três variantes do PTT5-small superam \textbf{todos} os
...
$\Delta \mathrm{F} = +13,4$ (vs.\ +13,2 do few-shot, diferença não
significativa, $p=0,87$), o GPT-4o mini CoT obtém
$\Delta \mathrm{F} = +12,0$ (vs.\ +10,3 do few-shot, também não
significativo, $p=0,13$), e o GPT-4o CoT obtém
$\Delta \mathrm{F} = +11,5$ (vs.\ +9,9 do few-shot, não
significativo, $p=0,19$). O Qwen 2.5 7B CoT, porém, reduz
significativamente o ganho de legibilidade
($\Delta \mathrm{F} = +3,34$ vs.\ +6,21 do few-shot, $p=0,019$),
sugerindo que o impacto do CoT na legibilidade varia conforme o
modelo. Assim, embora o CoT preserve a legibilidade (na maioria
dos modelos), ele reduz consistentemente a similaridade com a
referência nos quatro modelos testados, sugerindo que o
raciocínio explícito pode não beneficiar a tarefa de
simplificação textual nesse conjunto, possivelmente por esta
não exigir inferência complexa ou raciocínio simbólico. A
generalização para outros LLMs, incluindo Claude Sonnet 4.6
(não testado com CoT neste trabalho), permanece em aberto.
...
\begin{table*}[htbp]
\centering
\caption{Grupos de significância (correção de Holm-Bonferroni,
         $\alpha = 0,05$, bootstrap pareado, 10.000 iterações).
         Sobrescritos iguais = sem diferença significativa após
         correção.}
...
\toprule
Modelo & R-1 & SARI & $\Delta \mathrm{F}$ & $\Delta \mathrm{Fog}$ & R1$_{OP}$ & BS$_{OP}$ \\
\midrule
...
Em $\Delta \mathrm{F}$, os três PTT5 ($d$) produzem os menores ganhos de
...
prompting (GPT-4o mini, GPT-4o e Claude S. 4.6) em $\Delta \mathrm{F}$,
...
O Llama 4 Scout, apesar de liderar em $\Delta \mathrm{F}$, tem a fluência
mais baixa entre os modelos de prompting (4,80) por causa de
ocasionais quebras de estruturas gramaticais.

Os modelos ajustados em domínio revelam padrões qualitativos importantes.
...
variantes (2,73), coerente com seu maior $\Delta \mathrm{F}$ (+4,2).
O Qwen 2.5 0.5B FT, por sua vez, atinge a maior simplicidade entre
os ajustados em domínio (3,72), refletindo seu maior ganho de legibilidade,
...
o Gemini. Restringindo a análise aos cinco modelos de prompting
(Tabela~\ref{tab:llmjudge}), a média em simplicidade cai de
$4{,}64$ (Gemini) para $3{,}88$ (GPT-4o mini), em fluência de
$4{,}94$ para $4{,}45$, e em adequação de $4{,}80$ para
$4{,}22$. As
...
Para modelos ajustados em domínio (PTT5 e Qwen 0.5B), a concordância em
...
ajustados em domínio ($\alpha \geq 0,75$), sugerindo que a preservação de
...
média), mesmo para os modelos ajustados em domínio ($\kappa \approx 0,24$),
...
sistematicamente \citep{feinstein1990kappa}. Valores negativos de
$\kappa_w$ indicam, portanto, que os juízes divergem de forma
consistente (e não aleatória) em seus critérios, e não que a
concordância é melhor do que o acaso.
...
e ganho de legibilidade. O few-shot geralmente maximiza ambas as
métricas, embora haja exceções pontuais (e.g., Claude S. 4.6
zero-shot supera o few-shot em $\Delta \mathrm{F}$: +12,9 vs.\ +10,0).
...
(R1$_{OP} \approx 0,93$, $\Delta \mathrm{F} = +2,6$ a $+4,2$), praticamente
...
hipótese de underfitting: as curvas de treino e validação
estabilizam em plateaus distintos para todas as variantes,
indicando que o conservadorismo do PTT5-small não decorre de
underfitting nem de overfitting. Assim, uma poss\'\i{}vel
explicação para o comportamento conservador é uma
caracter\'\i{}stica inerente ao modelo, que por ter sido
pré-treinado para reconstruir o texto original, tende a
copiá-lo. O Qwen 2.5 0.5B, com arquitetura decoder-only, atinge
maior ganho de legibilidade ($\Delta \mathrm{F} = +9,6$) mas sofre de alucinações
...
\subsection{Robustez ao número de exemplos few-shot}

A escolha de k=3 representa um equililíbrio razoável entre
custo computacional e qualidade das simplificações, e é
consistente com valores adotados em estudos anteriores de
simplificação textual. Para verificar a sensibilidade dos
resultados ao número de exemplos, realizamos uma análise de
robustez com k = 1, 3, 5 e 7 no GPT-4o mini
(Tabela~\ref{tab:ablation}).
Os resultados mostram que as métricas de similaridade melhoram
com mais exemplos: SARI sobe de 39,3 (k=1) para 42,2 (k=7),
e R1 de 0,652 para 0,675.
O ganho de legibilidade ($\Delta \mathrm{F}$) varia entre +8,3 (k=7) e +10,7 (k=3),
sem tendência clara.
As diferenças são moderadas: a variação máxima é de
aproximadamente 8\% em SARI e 4\% em R1, e não alteram o
posicionamento relativo do GPT-4o mini em relação aos demais
modelos.

\begin{table*}[htbp]
\centering
\caption{Análise de robustez ao número de exemplos few-shot (k)
no GPT-4o mini.}
\label{tab:ablation}
\footnotesize
\begin{tabular}{c c c c c c}
\toprule
k & R1 & SARI & $\Delta \mathrm{F}$ & $\Delta \mathrm{Fog}$ & R1$_{OP}$ \\
\midrule
...
raciocínio (como DeepSeek e Nemotron) foram evitados devido ao
altíssimo consumo de tokens decorrente do raciocínio interno.
...
%
Este trabalho não conduziu validação humana em escala. Como os
juízes automáticos discordam fortemente
($\alpha$ médio $0{,}181$), a calibração das métricas automáticas
e do LLM-judge por meio de avaliadores humanos permanece em aberto.
Recomenda-se, em trabalhos futuros, uma anotação humana com ao
menos 2--3 avaliadores cobrindo 50--100 pares, com cálculo de
concordância inter-anotador (e.g., $\kappa$ de Cohen) e
comparação sistemática com as notas dos juízes LLM.

Sistemas de simplificação textual não-generativos para português
brasileiro (baseados em regras léxicas, morfológicas ou de
substituição de sinônimos) poderiam fornecer baselines
complementares para contextualizar os ganhos de legibilidade
reportados neste trabalho. A ausência de comparação direta
com tais sistemas é uma limitação deste estudo, mitigada
parcialmente pelo fato de que nosso foco é a comparação entre
paradigmas (prompting \emph{vs.}~fine-tuning) e não a obtenção
do estado da arte em simplificação para PT-BR.
...
treinada com uma única semente aleatória (sem variação de random
init ou ordem dos dados), o que impede separar variância
metodológica (inicialização, ordem do batch) de variância
estrutural (dados, hiperparâmetros) e, portanto, limita a
robustez das comparações entre variantes. Recomenda-se, em
trabalhos futuros, ao menos 3 sementes por configuração.

\section{Conclusão}

Este trabalho comparou cinco LLMs e
quatro modelos ajustados em domínio para simplificação textual em português
...
($\Delta \mathrm{F} \leq +4,2$). Métricas de similaridade e legibilidade devem
ser reportadas conjuntamente para uma avaliação completa.
Em aplicações que priorizam acessibilidade (e.g., leitura fácil),
legibilidade e o LLM-judge devem prevalecer sobre similaridade
lexical; em aplicações que seguem um padrão pré-definido, a
similaridade é preferível. A avaliação LLM-as-judge revela baixa
concordância entre juízes ($\alpha = 0,181$), indicando
sensibilidade à escolha do modelo juiz e limitando a
confiabilidade da avaliação automática isolada.
...
superiores; (iii) realizar avaliação com juízes humanos
para calibrar as métricas automáticas e o LLM-judge,
dada a baixa concordância inter-juiz observada
($\alpha = 0{,}181$ em média); (iv) investigar se prompts
estilísticos (com nível-alvo de simplificação explícito) podem
reduzir a lacuna entre similaridade e legibilidade em modelos
ajustados em domínio.
...
\iffalse
O commit base sobre o qual
esta versão foi editada é o de hash \texttt{5de9157} (24 jun.
2026); as edições da rodada de revisão interna (3 commits)
estão registradas no histórico de \texttt{git} do
repositório, mantendo reprodutibilidade experimental.
\fi
...
\textbf{João Pedro Hallack Sansão:} conceituação, metodologia, \emph{software},
investigação, curadoria de dados, análise formal, escrita -- rascunho original.
\textbf{Michel Carlo Rodrigues Leles:} conceituação, metodologia, supervisão,
escrita -- revisão e edição, administração do projeto.
\textbf{Iago Araújo de Oliveira:} conceituação, metodologia.
...
duas de maior divergência entre os modelos (\#103, com desvio padrão
do ROUGE-1 de 0,296; \#127, com 0,284) e a de maior comprimento
(\#141, 52 tokens). O desvio padrão é calculado sobre as pontuações
ROUGE-1 dos nove modelos em cada amostra.
...
\bigskip
\paragraph*{Amostra \#103 (divergência entre modelos, desvio padrão do ROUGE-1 = 0,296).}
...
\bigskip
\paragraph*{Amostra \#127 (segunda maior divergência entre modelos, desvio padrão do ROUGE-1 = 0,284).}
