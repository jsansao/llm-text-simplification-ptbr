...
%
\footnote{Sistemas de simplifica\c{c}\~ao textual n\~ao-generativos
(baseados em regras l\'exicas, morfol\'ogicas ou de
substitui\c{c}\~ao de sin\^onimos) para portugu\^es brasileiro
poderiam fornecer baselines complementares para contextualizar
os ganhos de legibilidade reportados neste trabalho. A
aus\^encia de compara\c{c}\~ao direta com tais sistemas \'e uma
limita\c{c}\~ao deste estudo, mitigada parcialmente pelo fato de
que nosso foco \'e a compara\c{c}\~ao entre paradigmas
(prompting \emph{vs.}~fine-tuning) e n\~ao a obten\c{c}\~ao do
estado da arte em simplifica\c{c}\~ao para PT-BR.}
...
\citep{cohen1988}. O valor de Cohen (1988) \'e citado como
aproxima\c{c}\~ao: an\'alises formais de poder para bootstrap
pareado com corre\c{c}\~ao de m\'ultiplas compara\c{c}\~oes est\~ao
...
identificado com esse problema.\footnote{Uma an\'alise de
sensibilidade (recalculo de R-1 e SARI excluindo a amostra
\#16) confirma que o par n\~ao altera qualitativamente as
conclus\~oes: Claude S. 4.6 (few-shot) passa de R-1 $= 0{,}661$
          ...
          {\tt Cópia} mostra o piso de similaridade (copiar o original
           como predição). A linha {\tt Ref.\ como pred.} mostra o
            SARI da referência humana contra o original (original como
             ...
             precis\~ao completa na Tabela~\ref{tab:significancia}.}
...
texto fonte. Vale notar que o SARI n\~ao possui, em si, um
...
n\~ao um limite superior absoluto para a tarefa.
...
manual das 200 amostras, identificamos duas sa\'idas em que o modelo
reproduz a instru\c{c}\~ao em vez de execut\'a-la (metatexto, 1\%):
a amostra \#16 produz ``Simplifique o texto a seguir: [\ldots]''
e a amostra \#141 produz apenas ``Aqui est\'a o texto
simplificado:''. Al\'em disso, doze sa\'idas (6\%) s\~ao c\'opias
literais do original. Esses casos ilustram uma fragilidade
conhecida do fine-tuning leve em modelos pequenos, mas n\~ao
alteram qualitativamente as m\'etricas agregadas reportadas.
...
indicando que o conservadorismo do PTT5-small n\~ao decorre de
underfitting nem de overfitting. Assim, uma poss\'\i{}vel
explica\c{c}\~ao para o comportamento conservador \'e uma
caracter\'\i{}stica inerente ao modelo, que por ter sido
pr\'e-treinado para reconstruir o texto original, tende a
copi\'a-lo. O Qwen 2.5 0.5B, com arquitetura decoder-only, atinge
...
A escolha de k=3 representa um equil\'ibrio razo\'avel entre
custo computacional e qualidade das simplifica\c{c}\~oes, e \'e
consistente com valores adotados em estudos anteriores de
simplifica\c{c}\~ao textual. Para verificar a sensibilidade dos
resultados ao n\'umero de exemplos, realizamos uma an\'alise de
...
init ou ordem dos dados), o que impede separar vari\^ancia
metodol\'ogica (inicializa\c{c}\~ao, ordem do batch) de vari\^ancia
estrutural (dados, hiperpar\^ametros) e, portanto, limita a
...
português acompanham o repositório. O commit base sobre o qual
