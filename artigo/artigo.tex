...
55,5), demonstrando que o treinamento em domínio específico compensa a
...
original meaning. This work evaluates five LLMs---GPT-4o mini, GPT-4o,
Claude Sonnet 4.6, Llama 4 Scout, and Qwen 2.5 7B---on text simplification
...
simplificação de sentenças, demonstrando que LLMs superam
...
Essa estratificação garante que a amostra preserve a distribuição
...
amostra (diferentes prompts), e cada execução LLM-as-judge consome
14--20 chamadas por amostra (9 modelos $\times$ 3 execuções),
totalizando milhares de chamadas de API além dos limites gratuitos
...
tópicos distintos (vacinação vs.\ servidora pública), indicando possível
erro de alinhamento; tal amostra foi mantida na análise por representar
...
garantir reprodutibilidade e comparação justa entre modelos, seguindo
...
passo antes de produzir a resposta final, com o objetivo de verificar
...
ROUGE-1 = 0,76 e BERTScore = 0,91, indicando que as simplificações
...
original produz SARI = 24,6, servindo como valor de referência
...
indicando que todos os prompts produzem simplificações que se afastam
moderadamente do original. O Llama 4 Scout, por outro lado, produz os
...
Claude S. 4.6 em ROUGE-1, confirmando que modelos abertos menores
...
+7,9 e redução Fog de $-1,9$ em relação ao original, servindo como
...
O Qwen 2.5 0.5B, por outro lado, apresentou R-1 de 0,60, SARI de 41,8
e F$\Delta = +9,6$, indicando simplificações menos similares
lexicalmente mas com ganhos de legibilidade substanciais, superando
...
QLoRA, não atinge o mesmo patamar, indicando que a escala reduzida
(0,5B parâmetros) limita o aprendizado mesmo com fine-tuning. Isso
demonstra que o fine-tuning em domínio específico pode superar a lacuna
...
enquanto Claude e GPT-4o atingem fluência máxima (5,00).
O Qwen 2.5 7B apresenta simplicidade ligeiramente inferior
(4,51), coerente com seu ganho de legibilidade mais modesto.
O Llama 4 Scout, apesar de liderar em F$\Delta$, tem a fluência
mais baixa entre os modelos de prompting (4,80), refletindo
...
sugerindo que modelos que maximizam similaridade lexical
...
sugerindo que maior similaridade lexical está associada a saídas
mais fluentes e adequadas, mas o efeito é pequeno. Esses resultados
indicam que as métricas automáticas e o LLM-judge capturam aspectos
...
Este resultado tem implicações importantes para a metodologia
LLM-as-judge: a escolha do modelo juiz impacta significativamente as
...
Um resultado particularmente relevante é que quatro modelos superam o
